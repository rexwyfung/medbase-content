# Burns Assessment and Resuscitation (燒傷評估與急救)
...
* **Industrial Burns**: Common in young working-class adults (chemical spills, high-voltage electrical injuries).
* **Major Burns Centres**: Severe cases in HK HA are centralized to **Queen Mary Hospital (QMH)** (HK Island/Kowloon West) or **Prince of Wales Hospital (PWH)** (NTE/Kowloon East).
...
* Systemic capillary leak (maximal at 8-12 hours post-burn) → fluid shift from intravascular to interstitial space (third-spacing) → **Hypovolaemic Shock (低血容量性休克)** + profound edema.
* ↑ evaporative water loss through compromised skin barrier.
* Hypermetabolic response → ↑ basal metabolic rate (can double), catabolism, and immunosuppression.
...
* Tense, woody feeling of the extremity.
* ↓ capillary refill, absent distal pulses (late sign).
* Tight, unyielding chest wall (restricts ventilation).

## 4. Differential Diagnosis & Discriminatory Features

### Classification of Burn Aetiology
| Aetiology | Discriminatory Features & Unique Management |
| :--- | :--- |
| **Thermal (Flame/Scald/Contact)** | Most common. Defined depth/surface area patterns. |
| **Chemical (Acids)** | Coagulative necrosis. Prevents deeper penetration. Rx: Copious water irrigation. |
| **Chemical (Alkalis)** | Liquefactive necrosis. Penetrates deeper. Rx: Prolonged copious water irrigation. |
| **Electrical (High Voltage)** | Massive deep muscle necrosis (iceberg effect), arrhythmias, compartment syndrome, rhabdomyolysis → myoglobinuria. |
...
* Exclude epidermal (1st degree) burns from TBSA calculation.
...
* **U&E**: ↑ K+ (massive cell lysis), acute kidney injury (AKI) from hypoperfusion/myoglobinuria.
* **ABG**: Assess oxygenation, ventilation, metabolic acidosis (lactic acidosis).
* **Carboxyhaemoglobin (COHb)**: Suspect CO poisoning in enclosed fires. Normal <2% (up to 10% in smokers). Toxic >15%.
* **CK & Myoglobin (Urine)**: Mandatory for high-voltage electrical injuries.
* **Coagulation Profile & G&S**: Baseline prior to surgical debridement.

### Imaging and Adjunct Workup
* **CXR**: Baseline; rule out blast injury/infiltrates (ARDS/pneumonia develops later).
* **ECG**: Arrhythmias (crucial in electrical burns).
* **Bronchoscopy**: Gold standard for confirming **Inhalation Injury (吸入性損傷)** (shows airway edema, erythema, soot).

## 6. Management Algorithm & Surgical Indications (Conservative, Medical, HA Protocols)

### Primary Survey & Immediate Management (ATLS)
* **A - Airway**: Early elective endotracheal intubation for inhalation injury/facial burns before severe edema obstructs airway.
* **B - Breathing**: 100% High-flow O2 (treats CO poisoning). Assess for circumferential chest burns.
* **C - Circulation**: 2 large-bore IV cannulae (unburnt skin if possible). Start fluid resuscitation.
* **D - Disability**: GCS assessment (hypoxia? CO toxicity?).
...
* Any full-thickness burns >5% TBSA.
...
### Nutritional Support & Metabolic Control
* Massive hypermetabolism. Target ↑ protein (2-3 g/kg/day) and calories (calculate via Galveston/Curreri formulas).
* Early enteral feeding (NG tube within 6-12 hours) preserves gut mucosal barrier and prevents bacterial translocation.
* Strict glycemic control (insulin infusion if necessary). Maintain warm ambient temperature (28-30°C).

### Burn Rehabilitation and Therapy
* Multi-disciplinary approach (PT/OT/Dietitian).
* **Positioning**: Splint joints in position of stretch/anti-contracture (e.g., neck extended, axilla abducted to 90°, hands in intrinsic plus position).
* **Pressure Garments**: Worn 23 hours/day for 12-18 months to flatten/prevent hypertrophic scars.

## 9. Surgical Complications (Early, Intermediate, Late) & Prognosis

### Early Complications (<48 hours)
* Airway compromise (edema).
* Hypovolaemic shock / distributive shock.
* Carbon monoxide (CO) and cyanide toxicity.
* Acute Compartment Syndrome.

### Intermediate Complications (Days to Weeks)
* **Sepsis**: Leading cause of death post-resuscitation. Burn wound infection (Pseudomonas aeruginosa, Staph aureus, fungal).
* ARDS, Acute Kidney Injury (AKI), Curling's ulcer (stress ulcer of duodenum - prevent with PPI).

### Late Complications (Months to Years)
* **Hypertrophic Scarring & Keloids**: Excessive collagen deposition.
* **Contractures (攣縮)**: Fibrosis across joints limiting range of motion; requires surgical release (Z-plasty).
* **Marjolin's Ulcer**: Squamous cell carcinoma arising in chronic burn scars (often decades later).
